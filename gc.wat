(module
  ...
  (func $get_size (param $ref i32) (result i32)
    (i32.sub (call $memory_get_size (call $to_memory_pointer (get_local $ref))) (get_global $HEAD_SIZE))
  )
  ...
  (func $mark_rec (param $ref i32)
    (local $i i32)
    ;;使用中
    (if (i32.eq (call $memory_get_flag (call $to_memory_pointer (get_local $ref))) (get_global $memory_USE_FLAG_USE))
      (then
        ;;マークされてない
          ...
          (then
            ...
            (if (call $get_bit_flag (get_local $ref) (get_global $FLAG_IS_POINTERS))
              (then
                ...
                loop $loop
                  (if (i32.lt_s (get_local $i) (i32.div_s (call $get_size (get_local $ref)) (i32.const 4)))
                    (then
  ...
  (func $sweep
    ...
    loop $loop
      (if (i32.ne (call $memory_get_flag (get_local $iter_p)) (get_global $memory_USE_FLAG_INVALID))
        (then
          (if (call $get_bit_flag (call $from_memory_pointer (get_local $iter_p)) (get_global $FLAG_MARKED))
            (then
              ;;マークしてるならマーク外す
              (call $off_bit_flag (call $from_memory_pointer (get_local $iter_p)) (get_global $FLAG_MARKED))
            )
            (else
              ;;マークしてないなら解放
              (call $free (call $from_memory_pointer (get_local $iter_p)))
            )
          )
          (set_local $iter_p (call $memory_get_next (get_local $iter_p)))
          br $loop
